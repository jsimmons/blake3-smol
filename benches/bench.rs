...
fn bench_single_compression_fn(b: &mut Bencher, platform: Platform) {
    ...
    let input = r.get()[0..64].try_into().unwrap();
...
fn bench_many_chunks_fn(b: &mut Bencher, platform: Platform) {
    ...
    let input_arrays: Vec<&[u8; CHUNK_LEN]> = inputs
        .iter_mut()
        .take(degree)
        .map(|i| i.get()[0..CHUNK_LEN].try_into().unwrap())
        .collect();
    let input_arrays = input_arrays.as_slice();

    b.iter(|| {
        black_box(input_arrays);
...
fn bench_many_parents_fn(b: &mut Bencher, platform: Platform) {
    ...
    let input_arrays: Vec<&[u8; BLOCK_LEN]> = inputs
        .iter_mut()
        .take(degree)
        .map(|i| i.get()[0..BLOCK_LEN].try_into().unwrap())
        .collect();
    let input_arrays = input_arrays.as_slice();
